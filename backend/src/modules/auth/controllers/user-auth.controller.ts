...
export class UserAuthController {
  ...
  async login(
    ...
    return this.authService.userLogin(Number(tenantId), loginDto.username, loginDto.password);
  }
  ...
  @ApiResponse({ status: 401, description: 'Refresh token không hợp lệ' })  async refreshToken(
    ...
    return this.authService.refreshToken(refreshTokenDto.token, 'user', tenantId);
  }
  ...
  @ApiResponse({ status: 200, description: 'Đăng xuất thành công' })  async logout(
    ...
    return this.authService.logout(userId, 'user', tenantId);
  }
}
